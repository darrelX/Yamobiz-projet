...
3. **Configurer les secrets des Edge Functions** (jamais dans le frontend).
   Récupère ces valeurs directement sur developers.facebook.com (App Meta →
   WhatsApp → API Setup) — **ne les copie pas depuis un fichier du
   backend**, même si tu choisis d'utiliser le même numéro Meta. Ces
   fonctions n'ont aucune dépendance envers le backend, seule la base
   Supabase est commune :
   ```bash
   supabase secrets set WHATSAPP_ACCESS_TOKEN=xxxx
   supabase secrets set WHATSAPP_PHONE_NUMBER_ID=xxxx
   supabase secrets set WHATSAPP_API_VERSION=v21.0   # optionnel, v21.0 par défaut si omis
   ```
